...
public class AdminController : ControllerBase
{
    ...
    public async Task<IActionResult> GetOverview()
    {
        ...
        // Reflect the *actual* last run per source instead of a hardcoded "Ready" — each source
        // now really calls its external API, so this can genuinely be "Failed" if that API is down
        // or "Never run" if the scheduled job hasn't executed yet in this environment.
        var apiHealth = new List<object>();
        foreach (var source in new[] { "SemanticScholar", "OpenAlex", "GoogleScholar" })
        {
            var latestForSource = await _context.SyncLogs
                .Where(s => s.SourceApi == source)
                .OrderByDescending(s => s.StartedAt)
            ...
            {
                label = source,
                value = latestForSource == null ? "Never run" : latestForSource.Status.ToString(),
                lastRunAt = latestForSource?.StartedAt
            });
        }
    ...
    public async Task<IActionResult> GetUsers([FromQuery] int page = 1, [FromQuery] int pageSize = 20)
    {
        ...
        // Projected in-memory (not inside the EF query above) because HasProAccess()/IsInFreeTrial()
        // are custom extension methods that EF Core cannot translate into SQL.
        var users = pagedEntities.Select(u => new
        {
            id = u.Id,
            name = u.FullName,
            fullName = u.FullName,
            ...
            isActive = u.IsActive,
            isPro = u.IsPro,
            isTrialActive = u.IsInFreeTrial(),
            plan = u.IsPro ? "Pro" : (u.IsInFreeTrial() ? "Pro (Free Trial)" : "Free"),
            searchAccuracy = PlanPolicy.GetSearchAccuracy(u.Role, u.HasProAccess()),
    ...
    public async Task<IActionResult> BroadcastNotification([FromBody] AdminBroadcastNotificationDto request)
    {
        ...
        for (var index = 0; index < users.Count; index++)
        {
            ...
            {
                // Notifications are already saved to the database at this point, so a single
                // recipient's SignalR push failing (e.g. they are offline) must not fail the
                // whole broadcast request or cause the admin to re-send it and duplicate rows.
            }
        }
    ...
    [HttpPost("sync/google-scholar")]
    public async Task<IActionResult> SyncGoogleScholar()
    {
        await LogAuditAsync("Sync Management", "Admin triggered Google Scholar sync.", "Info", "ADMIN-SYNC-GOOGLE-SCHOLAR");
        await _syncService.SyncFromGoogleScholarAsync();
        return Ok(new { message = "Google Scholar sync started." });
    }
    ...
    {
        var query = _context.SyncLogs.Where(l => l.SourceApi != "PlanPolicy");
