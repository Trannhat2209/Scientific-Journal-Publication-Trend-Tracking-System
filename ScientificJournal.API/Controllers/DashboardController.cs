...
public class DashboardController : ControllerBase
{
    ...
    public async Task<IActionResult> GetUserSummary([FromServices] AppDbContext context, [FromServices] IRecommendationService recommendationService)
    {
        ...
        return Ok(new
        {
            ...
            isPro = dbUser.IsPro,
            isTrialActive = dbUser.IsInFreeTrial(),
            plan = dbUser.IsPro ? "Pro" : (dbUser.IsInFreeTrial() ? "Pro (Free Trial)" : "Free"),
            searchAccuracy = PlanPolicy.GetSearchAccuracy(dbUser.Role, dbUser.HasProAccess()),
            bookmarksCount,
            followsCount,
            unreadAlerts,
            recommendations
