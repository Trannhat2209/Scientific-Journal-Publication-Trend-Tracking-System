...
const isStoredAuthTokenValid = () => {
  const auth = getStoredAuth();
  if (!auth.accessToken) return false;

  const tokenPayload = decodeJwtPayload(auth.accessToken);
  const now = Date.now();
  const tokenExpiry = Number(tokenPayload.exp || 0);
  if (tokenExpiry && now >= tokenExpiry * 1000) return false;
...
const normalizeOptionalRoleForUi = (role) => {
  const value = Array.isArray(role) ? role[0] : role;
  if (!String(value || "").trim()) return "";
  return normalizeRoleForUi(value);
};

const protectedRoutePolicies = [
  {
    match: (path) => path.startsWith("/admin-"),
    roles: ["Administrator"],
    requireBackendToken: true,
  },
  {
  ...
  {
    match: (path) => path.startsWith("/lecturer-"),
    roles: ["Lecturer"],
    requireBackendToken: true,
  },
  {
    match: (path) => path.startsWith("/researcher-"),
    roles: ["Researcher"],
    requireBackendToken: true,
  },
];

const getRouteAccessPolicy = (path) =>
  protectedRoutePolicies.find((policy) => policy.match(path)) || null;

const getAuthenticatedAccount = () => {
  const session = getStoredSession();
  const hasBackendToken = isStoredAuthTokenValid();
  const role = normalizeOptionalRoleForUi(
    hasBackendToken ? getStoredAuthRole() : session.role,
  );
  const isSignedIn = Boolean(hasBackendToken || session.email || session.id);
  ...
  return {
    ...
    isSignedIn,
    hasBackendToken,
    dashboardRoute: roleDashboardRoutes[role] || "/login",
  };
};

const getRouteGuardDecision = (path) => {
  ...
  if (policy.requireBackendToken && !account.hasBackendToken) {
    return {
      allowed: false,
      reason: "backend-auth",
      policy,
  ...
  if (!policy.roles.includes(account.role)) {
    return {
      allowed: false,
      reason: "role",
      policy,
      account,
      redirectRoute: account.dashboardRoute,
    };
  }

  return { allowed: true, policy, account };
};
...
function LoginPage() {
  ...
  const backendLoginPresets = {
    Researcher: {
      email: "le.researcher@uni.edu.vn",
    ...
    Lecturer: {
      email: "tran.lecturer@hcmus.edu.vn",
      password: "Scholar2024",
      role: "Lecturer",
      fullName: "Tran Quoc Binh",
  ...
  const roleEmails = Object.fromEntries(
    Object.entries(backendLoginPresets).map(([role, account]) => [
      role,
      account.email,
    ]),
  );
  ...
  const loginWithCredentials = async ({
    ...
    try {
      ...
      const selectedLoginRole = normalizeRoleForUi(requestedRole);
      if (role !== selectedLoginRole) {
        clearAuth();
        setIsLoggingIn(false);
        setFeedback({
          type: "error",
          text: `This account is registered as ${role}. Please select ${role} to sign in.`,
        });
        return;
      }
  ...
  const handleRoleSelect = (nextRole) => {
    const account = backendLoginPresets[nextRole];
...
const normalizeAdminManagedUser = (user) => ({
  ...
  name: user.name || user.fullName || "Unnamed User",
  email: user.email || "",
  role: normalizeRoleForUi(user.role || "Researcher"),
  ...
  searchAccuracy:
    ...
    getSearchAccuracyForAccount(
      normalizeRoleForUi(user.role || "Researcher"),
...
function AdminUserManagementPage() {
  ...
  const adminAccessMessage =
    "A valid backend Administrator session is required to manage users.";
  ...
  const loadBackendUsers = React.useCallback(async () => {
    ...
    try {
      ...
      setMessage(
        backendUsers.length
          ? ""
          : "No backend users found for this administrator account.",
      );
      return backendUsers;
    } finally {
                  ...
                  <tr>
                    <td colSpan="7" className="admin-users-empty">
                      {isLoadingUsers
                        ...
                        : "No backend users match these filters."}
                    </td>
                  </tr>
                ) : null}
              </tbody>
...
function RouteGuardPage({ decision }) {
  const account = decision?.account || {};
  const requiredRoles = decision?.policy?.roles?.join(", ") || "a permitted role";
  const isBackendAuthRequired = decision?.reason === "backend-auth";
  const title =
  ...
  const message =
    decision?.reason === "auth"
      ...
      : isBackendAuthRequired
        ? `${requiredRoles} pages require a valid backend session.`
        : `This page is available for ${requiredRoles} accounts.`;
  const dashboardRoute =
    account.dashboardRoute && account.dashboardRoute !== "/login"
      ? account.dashboardRoute
      : "";

  return (
    <main className="payment-return-page">
      <section className="payment-return-card error">
        <div className="upgrade-modal-mark" aria-hidden="true">
          <MiniIcon path="M12 3.5 19 6.5v5.2c0 4.2-2.8 7.3-7 8.8-4.2-1.5-7-4.6-7-8.8V6.5l7-3ZM12 8.5v4M12 15.8h.01" />
        </div>
        <span>ScholarTrend Access</span>
        <h1>{title}</h1>
        <p>{message}</p>
        <button type="button" onClick={navTo("/login")}>
          Go to Login
        </button>
        {dashboardRoute ? (
          <button type="button" onClick={navTo(dashboardRoute)}>
...
export default function App() {
  ...
  const routeGuardDecision = getRouteGuardDecision(path);
  if (!routeGuardDecision.allowed) {
    return <RouteGuardPage decision={routeGuardDecision} />;
  }
